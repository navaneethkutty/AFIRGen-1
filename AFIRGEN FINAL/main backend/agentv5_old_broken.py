...
from infrastructure.cloudwatch_metrics import (
    get_metrics,
    record_api_request,
...
# Import X-Ray tracing (removed - not needed for AWS Bedrock deployment)
# X-Ray tracing removed - not needed for AWS Bedrock deployment
...
# Stub functions for removed X-Ray tracing
def setup_xray(*args, **kwargs):
    pass
def trace_subsegment(name):
    def decorator(func):
        return func
    return decorator
def add_trace_annotation(*args, **kwargs):
    pass
def add_trace_metadata(*args, **kwargs):
    pass
...
# Background task management removed - not needed for AWS Bedrock deployment
...
# Import Bedrock services (commented out - files don't exist yet)
...
# Stub function for removed Bedrock services
def get_settings():
    class Settings:
        enable_bedrock = False
        class aws:
            region = "us-east-1"
            s3_bucket = "afirgen-bucket"
        class bedrock:
...
# Vector DB and cache services removed - not needed for AWS Bedrock deployment
...
# RetryHandler already exists in infrastructure, import from there
from infrastructure.retry_handler import RetryHandler

# Resilience and monitoring services removed - not needed for AWS Bedrock deployment
...
# Use stubs for removed services
BedrockCircuitBreaker = CircuitBreaker  # Use the stub we created earlier
BedrockMetricsCollector = MetricsCollector  # Use infrastructure MetricsCollector

def get_structured_logger(name):
    return log  # Use the existing logger
...
async def find_violations_with_validation(session_id: str, summary: str, user_input: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if settings.enable_bedrock:
        ...
        # TODO: Replace with MySQL-based IPC section retrieval (Task 4.5)
        # ChromaDB has been removed - need to implement SQL LIKE queries
        raise NotImplementedError(
            "ChromaDB has been removed. IPC section retrieval needs to be "
            "reimplemented using MySQL with SQL LIKE queries. See Task 4.5."
        )
...
# Initialize FIR service (background task manager removed)
fir_service = FIRService()

# Background tasks removed - not needed for AWS Bedrock deployment
...
# CORS middleware - using FastAPI built-in (enhanced middleware removed)
...
# Metrics middleware removed - not needed for AWS Bedrock deployment
...
async def validate_step(validation_req: ValidatedValidationRequest):
    ...
    try:
        ...
        if current_step == ValidationStep.TRANSCRIPT_REVIEW:
            ...
            if fir_number:
                log.info(f"FIR {fir_number} completed successfully")
                # Background task functionality removed
                # try:
...
async def authenticate_fir(auth_req: ValidatedAuthRequest):
    ...
    try:
        ...
        log.info(f"FIR {auth_req.fir_number} finalized successfully")
        # try:
